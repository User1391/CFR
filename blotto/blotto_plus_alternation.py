...
def compute_exploitability(strategies, avg_strategy, game_eval_func):
        """
        Given a list of pure strategies, a mixed strategy (avg_strategy),
        and a game evaluation function: compute the exploitability.

        Exploitability = max best response payoff - expected payoff vs self
        """
        # Compute best response utility
        br_utilities = []
        for strat in strategies:
            utility = 0
            for j, opp_strat in enumerate(strategies):
                payoff = game_eval_func(strat, opp_strat)
                utility += avg_strategy[j] * payoff
            br_utilities.append(utility)

        best_response_value = max(br_utilities)

        # Compute expected value of playing the strategy against itself
        expected_value = 0
        for i, strat1 in enumerate(strategies):
            for j, strat2 in enumerate(strategies):
                expected_value += avg_strategy[i] * avg_strategy[j] * game_eval_func(strat1, strat2)

        exploitability = best_response_value - expected_value
...
class Player:
    ...
    def update_strategy(self):
        ...
        for a in range(size):
            ...
            self.strategy_sum[a] += self.iteration * self.strategy[a] 
        self.iteration += 1
    ...
    def update_learning(self, game, self_action, opponent_action):
        ...
        for a in range(len(self.strategies)):
            self.regret_sum[a] = max(0, self.regret_sum[a] + game.evaluate_game(self.strategies[a], opponent_action) - my_action_utility)
...
class Trainer:
    ...
    def iterate_alternation(self, iteration=0):
        self.p1.update_strategy()
        self.p2.update_strategy()
        p1_action = self.p1.select_action()
        p2_action = self.p2.select_action()
    ...
    def train_smart(self, max_iterations, threshold=0.01):
        for i in tqdm(range(max_iterations)):
            self.iterate()
            if i % 100 == 0:  # check every 100 iterations
                exp = self.compute_exploitability()
                if exp < threshold:
                    print(f"Converged at iteration {i} with exploitability {exp:.5f}")
                    break
